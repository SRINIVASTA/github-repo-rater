...
if st.button("Analyze Profile Metrics", type="primary"):
    if not user_input:
        ...
        headers = {
            ...
            "User-Agent": "GitScore-Enterprise-v6"
        }
        if "GITHUB_TOKEN" in st.secrets and st.secrets["GITHUB_TOKEN"].strip() != "":
        ...
        with st.spinner(f"Requesting data streams for @{username}..."):
            try:
                # Wrapped inside a try block to handle raw proxy drops cleanly
                ...
                if response.status_code == 200:
                    ...
                    st.info("Your `GITHUB_TOKEN` is invalid. Regenerate a new token on GitHub and update your Streamlit Secrets.")
                elif response.status_code == 403:
                    st.error("🚫 Access Denied / Rate Limit Hit (Error 403)")
                    st.info("Your token lacks permissions. Go to GitHub settings, edit your token, and make sure the **public_repo** check box is checked.")
                elif response.status_code == 404:
                    st.error(f"🔍 User Not Found (Error 404)")
                    st.info(f"The GitHub username `{username}` does not exist. Check your spelling.")
                else:
                    st.error(f"🌐 Infrastructure Error Code: {response.status_code}")

            except (requests.exceptions.ConnectionError, requests.exceptions.Timeout):
                st.error("🌐 Network Connection Error!")
                st.info(
                    "Streamlit Cloud is hitting a proxy block with GitHub. "
                    "This usually means the `GITHUB_TOKEN` value pasted in your Streamlit Secrets has a typo, "
                    "an extra space, or is an expired token string. Please double-check it!"
                )
